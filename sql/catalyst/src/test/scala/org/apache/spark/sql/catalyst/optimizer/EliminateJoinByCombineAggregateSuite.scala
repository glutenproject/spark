...
class EliminateJoinByCombineAggregateSuite extends PlanTest {
  private object Optimize extends RuleExecutor[LogicalPlan] {
    val batches =
      Batch("Eliminate Join By Combine Aggregate", FixedPoint(10),
        ...
        RemoveNoopOperators,
        PushDownPredicates,
        EliminateJoinByCombineAggregate) :: Nil
  }
  ...
  override def test(testName: String, testTags: Tag*)(testFun: => Any)
    ...
    super.test(testName, testTags: _*) {
      withSQLConf(SQLConf.ELIMINATE_JOIN_BY_COMBINE_AGGREGATE_ENABLED.key -> "true") {
        testFun
      }
    }
  }
  ...
  test("join side contains Aggregate and aggregate expressions exist Filter clause") {
    val originalQuery1 =
    ...
    val originalQuery2 =
      testRelation.where(a === 1).groupBy()(sum(b).as("sum_b")).join(
        testRelation.where(a === 2).groupBy()(sum(b, Some(c === 1)).as("sum_b")))
