    ...
    def extract(
      ...
      case ExtractEquiJoinKeys(_, lkeys, rkeys, _, _, left, right, _) =>
        ...
        if (left.output.exists(_.semanticEquals(currentFilterCreationSideExp))) {
          ...
          // There may be a transfer of creation side expressions for runtime filters. If one side
          // of the join condition contains the current creation side expression, the expression on
          // the other side of the join condition can be used as the passed creation side
          // expression.
          if (extracted.isEmpty) {
            lkeys.zipWithIndex.find { case (key, _) =>
              currentFilterCreationSideExp.semanticEquals(key)
            }.map { case (_, idx) =>
              val passedFilterCreationSideExp = rkeys(idx)
          ...
          val extracted = extract(right, AttributeSet.empty,
            ...
            currentFilterCreationSideExp = currentFilterCreationSideExp)
          if (extracted.isEmpty) {
            rkeys.zipWithIndex.find { case (key, _) =>
              currentFilterCreationSideExp.semanticEquals(key)
            }.map { case (_, idx) =>
              val passedFilterCreationSideExp = lkeys(idx)
